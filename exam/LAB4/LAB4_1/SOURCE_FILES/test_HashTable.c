...
void newTest(){
    ...
    htable = changeHashTableSize(&htable, 40); //Change size of empty table
    assert(getSize(&htable) == 40);
    htable = changeHashTableSize(&htable, 40); //Change size to same size. 
    assert(getSize(&htable) == 40);
    ...
    assert(getElementsInTable(&htable) == 9);

    htable = changeHashTableSize(&htable, 9); //Change size to exact number of elements

    assert(getElementsInTable(&htable) == 9);

    htable = changeHashTableSize(&htable, 2); //Try to set size less than ammount of elements. 

    assert(getSize(&htable) == 9);



    printf("Passed new test. \n");
}

...
void testPostfix()
{
    assert(calculatePostfixExpression("483*+") == 28);
    assert(calculatePostfixExpression("48+3*") == 36);
    assert(calculatePostfixExpression("48-3+") == -1);
    assert(calculatePostfixExpression("83/3+") == 5);
    assert(calculatePostfixExpression("231*+9-") == -4);
    assert(calculatePostfixExpression("62/12+*") == 9);
    printf("Passed postfix test! \n");
}
...
int calculatePostfixExpression(char postfix[])
{
    ...
    {                              // Loop through the string
        char current = postfix[i]; //Solve multiple int problem
        if (isdigit(current))
        { // If it's a digit, add to stack
            push(&stack, atoi(&current));
        }
        // if it's a operator, pop top 2 and perform operation and then add it back to the stack
        else if (current == '+')
        {
            int x = peekStack(stack);
            pop(&stack);
            int y = peekStack(stack);
            pop(&stack);
            push(&stack, y + x);
        }
        else if (current == '-')
    ...
    printf("Calculation: %-10s result: %d\n", postfix, peekStack(stack));
